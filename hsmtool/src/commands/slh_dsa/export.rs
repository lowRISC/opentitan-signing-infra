...
use crate::util::key::slhdsa::{
    SlhDsaPrivateKey, SlhDsaPublicKey, save_private_key, save_public_key,
};
...
#[typetag::serde(name = "slh-dsa-export")]
impl Dispatch for Export {
    fn run(
        ...
        if self.wrap.is_some() {
            ...
            let sk = SlhDsaPrivateKey::try_from(&map)?;
            save_private_key(&self.filename, &sk, self.format)?;
        } else {
            let map = AttributeMap::from_object(session, object)?;
            let pk = SlhDsaPublicKey::try_from(&map)?;

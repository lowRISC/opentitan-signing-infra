...
"""Signing macros / rules for framing signing operations.

This includes generating pre-signing artifacts before the signing operation takes place,
and attaching signatures that are generated by the signing operation to the unsigned
binary.
"""
...
def presigning_artifacts(ctx, opentitantool, src, manifest_attr, ecdsa_key, rsa_key, spx_key, basename = None, keyname_in_filenames = False):
    ...
    digests/messages required for signing, and a JSON "script" containing
    signing directives for automated signign flows.

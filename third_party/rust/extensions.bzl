...
def _serde_annotate_repo():
    # TODO: investigate if there is a a better solution we can use to avoid needing to
    # apply this patch whilst using a different crate_universe name.
    http_archive(
        ...
        patches = ["//third_party/rust/patches:serde-annotate.rename_crate_universe.patch"],
        patch_args = ["-p1"],
    )
